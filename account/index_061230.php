...
<?php
	if(!$accountrow){
		...
		for($i=0;$i<count($fieldnames);$i++){
			...
			if($fieldnames[$i]!='accountid'){
				...
				echo "<td>";
				if($fieldnames[$i]=="password"){echo "<input type=\"password\" name=\"".$fieldnames[$i]."\" size=\"70\"/>";}
				elseif($fieldnames[$i]=="phone_number" || $fieldnames[$i]=="fax"){echo "<input type=\"text\" name=\"".$fieldnames[$i]."\" size=\"70\" onkeyup=\"inputmask(this.value,this,'phone');\" maxlength=\"12\"/>";}
				elseif($fieldnames[$i]=="zip"){echo "<input type=\"text\" name=\"".$fieldnames[$i]."\" size=\"70\" onkeyup=\"inputmask(this.value,this,'zip');\" maxlength=\"5\"/>";}
				...
				else{echo "<input type=\"text\" name=\"".$fieldnames[$i]."\" size=\"70\"/>";}
				echo "</td>";
				echo "</tr>";
			}
		}
	...
	for($i=0;$i<count($fieldnames);$i++){
		...
		if($fieldnames[$i]=='accountid'){
			...
			echo "<td>".str_replace(" ","&nbsp;",ucwords(str_replace("_"," ",$fieldnames[$i]))).":</td>";
			echo "<td>";
			if($fieldnames[$i]=="password"){echo "<input type=\"password\" name=\"".$fieldnames[$i]."\" value=\"".formvalue($row[$fieldnames[$i]])."\" size=\"70\"/>";}
			elseif($fieldnames[$i]=="phone_number" || $fieldnames[$i]=="fax"){echo "<input type=\"text\" name=\"".$fieldnames[$i]."\" value=\"".formvalue($row[$fieldnames[$i]])."\" size=\"70\" onkeyup=\"inputmask(this.value,this,'phone');\" maxlength=\"12\"/>";}
			elseif($fieldnames[$i]=="zip"){echo "<input type=\"text\" name=\"".$fieldnames[$i]."\" value=\"".formvalue($row[$fieldnames[$i]])."\" size=\"70\" onkeyup=\"inputmask(this.value,this,'zip');\" maxlength=\"5\"/>";}
			elseif(get_field_type("accounts",$fieldnames[$i])=="enum"){
				echo "<select name=\"".$fieldnames[$i]."\">";
				$vals=get_enum("accounts",$fieldnames[$i]);
				for($j=0;$j<count($vals);$j++){
			...
			else{echo "<input type=\"text\" name=\"".$fieldnames[$i]."\" value=\"".formvalue($row[$fieldnames[$i]])."\" size=\"70\"/>";}
			echo "</td>";
			echo "</tr>";
		}
	}
