...
<?php
	...
	elseif($action=='createaccount'){
		...
		if($row["accountid"]){$error="An account matching this email already exists.";}
			...
			@mysql_query("INSERT INTO accounts (email,password,first_name,last_name,phone_number) VALUES (\"".trim($email)."\",\"".trim($password)."\",\"".trim($first_name)."\",\"".trim($last_name)."\",\"".trim($phone_number)."\")");
	...
	elseif($action=='updateaccount'){
		...
		if($row["accountid"]){$error="Another account is already using this email address ($email).";}
			...
			$sql.=", first_name=\"".trim($first_name)."\"";
			$sql.=", last_name=\"".trim($last_name)."\"";
...
function verify_create(f){
	...
	if(trim(document.createaccount.first_name.value)==''){msg=msg+'\n('+index+') Please enter your first name.';index++;}
	if(trim(document.createaccount.last_name.value)==''){msg=msg+'\n('+index+') Please enter your last name.';index++;}
...
function verify_update(f){
	...
	if(trim(document.updateaccount.first_name.value)==''){msg=msg+'\n('+index+') Please enter your first name.';index++;}
	if(trim(document.updateaccount.last_name.value)==''){msg=msg+'\n('+index+') Please enter your last name.';index++;}
...
<?php
	if(!$accountrow){
		...
		echo "<tr><td>Email:</td><td><input type='text' name='email'/></td></tr>";
		echo "<tr><td>Password:</td><td><input type='password' name='password'/></td></tr>";
		echo "<tr><td>First Name:</td><td><input type='text' name='first_name'/></td></tr>";
	...
	echo "<tr><td>First Name:</td><td><input type='text' name='first_name' value=\"".formvalue($accountrow["first_name"])."\"/></td></tr>";
	echo "<tr><td>Last Name:</td><td><input type='text' name='last_name' value=\"".formvalue($accountrow["last_name"])."\"/></td></tr>";
