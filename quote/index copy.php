...
<script>
	function verify(f){
		...
		var filecheck=true;
		if(!checkextention(document.quote1.design_file_1.value)){filecheck=false;}
		if(!checkextention(document.quote1.design_file_2.value)){filecheck=false;}
		if(!checkextention(document.quote1.design_file_3.value)){filecheck=false;}
		if(!checkextention(document.quote1.design_file_4.value)){filecheck=false;}
		if(!checkextention(document.quote1.design_file_5.value)){filecheck=false;}
		if(!checkextention(document.quote1.design_file_6.value)){filecheck=false;}
		if(!checkextention(document.quote1.design_file_7.value)){filecheck=false;}
		if(!checkextention(document.quote1.design_file_8.value)){filecheck=false;}
		if(!checkextention(document.quote1.design_file_9.value)){filecheck=false;}
		if(!checkextention(document.quote1.design_file_10.value)){filecheck=false;}

			
		if(!filecheck){msg=msg+'\n('+index+') Please upload valid design file(s).  Only .zip, .ai, .eps., .dxf, .scv, .dwg, and .pdf files are allowed.';index++;}
	...
	function checkextention(file){
		var last3=file;
		if(trim(last3)==''){return(true);}
		last3=last3.substr(last3.length-3);
		last3=last3.toUpperCase();
		if(!(last3=='ZIP' || last3=='.AI' || last3=='EPS' || last3=='DXF' || last3=='SCV' || last3=='DWG' || last3=='PDF')){return(false);}
		return(true);
	}
</script>
...
<?php
	...
	echo "Please complete the quote request form in as much detail as possible and click the \"Request Quote\" button to submit your request.  We strive to turn quotes around in two business days.<br>&nbsp;";	
	...
	echo "<tr><td colspan=\"2\">&nbsp;<br><i>Please review our drawing <a href=\"#\" onclick=\"window.open('/filespecs.php','filespecs','status=0,toolbar=0,location=0,resizeable=1,scrollbars=1,menubar=0,directories=0,height=500,width=600');\">file requirements</a>.</i>";
	for($i=1;$i<=10;$i++){
		echo "<tr><td>Design File #$i</td><td><input type=\"file\" name=\"design_file_$i\"/></td></tr>";
	}
